...
DEADLINE_PATTERN = re.compile(r"(오늘마감|\d+일남음)")
APPLY_PATTERN = re.compile(r"신청\s*([\d,]+)명\s*/\s*([\d,]+)명")
PRICE_PATTERN = re.compile(r"제공내역\s*([\d,]+원)")
...
def clean_title(text: str) -> str | None:
    ...
    text = re.sub(r"^일남음\s*", "", text).strip()
    text = normalize_text(text)
...
class StyleCCrawler(BaseCrawler):
    def crawl(self) -> list[Campaign]:
        ...
        unique: dict[tuple[str | None, str | None, str | None, str | None, str | None], Campaign] = {}
